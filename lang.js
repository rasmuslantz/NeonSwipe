// i18n + pricing display + animations (safe on all pages)
...
const L = {
  en: {
    ...
    s_q_perm_t:"Allow Photos access",
    s_q_perm_d:"iOS: Settings → Privacy & Security → Photos → Neon Swipe → Allow “All Photos”.",
    s_q_undo_t:"Undo a deletion",
    s_q_undo_d:"Photos → Albums → Recently Deleted (available ~30 days).",
    s_q_space_t:"Storage not changing?",
    s_q_space_d:"After a cleanup, empty “Recently Deleted” to reclaim space immediately.",
    s_q_restore_t:"Restore purchases",
    s_q_restore_d:"Open Neon Swipe → Settings → Restore Purchases.",
    s_contact_title:"Contact us",
    s_contact_desc:"Can’t find what you need? Email us and include your device model and iOS/Android version.",
    s_email_label:"Email",
    s_email_value:"neonswipe-support@gmail.com",
    ...
    p_diag_t:"Diagnostics",
    p_diag_d:"If you opt in to share diagnostics with Apple/Google or send us logs via support, we may receive anonymized crash reports and technical data used only to fix issues and improve performance.",
    p_security_t:"Security",
    p_security_d:"We apply industry-standard safeguards to any limited data we hold (e.g., support emails). Where feasible, data is encrypted in transit and at rest.",
    p_third_t:"Third-party partners",
    p_third_d:"When you interact with ads or make a purchase, the relevant partners (ad networks, Apple, Google) process your data under their own policies.",
    p_region_t:"International transfers",
    p_region_d:"Because ads and billing are provided by global platforms, your information may be processed outside your country. Those providers implement their own safeguards to facilitate such transfers.",
    p_ret_t:"Retention",
    p_ret_d:"Support emails and related correspondence are retained only as long as necessary to resolve your request and meet legal obligations.",
    p_changes_t:"Changes to this policy",
    p_changes_d:"We may update this notice from time to time. Material changes will be posted here with a new effective date.",
    p_contact_t:"Contact",
    p_contact_d:'Questions about privacy? Email <a href="mailto:neonswipe-support@gmail.com">neonswipe-support@gmail.com</a>.',
    p_effective_t:"Effective date",
    p_effective_d:"This policy is effective as of 1 October 2025."
  },
  ...
  es: {
    ...
    s_q_perm_t:"Permitir acceso a Fotos",
    s_q_perm_d:"iOS: Ajustes → Privacidad y seguridad → Fotos → Neon Swipe → “Todas las fotos”.",
    s_q_undo_t:"Deshacer un borrado",
    s_q_undo_d:"Fotos → Álbumes → Eliminado recientemente (≈30 días).",
    s_q_space_t:"¿El espacio no cambia?",
    s_q_space_d:"Tras limpiar, vacía “Eliminado recientemente” para recuperar espacio al instante.",
    s_q_restore_t:"Restaurar compras",
    s_q_restore_d:"Neon Swipe → Ajustes → Restaurar compras.",
    s_contact_title:"Contáctanos",
    s_contact_desc:"¿No encuentras lo que necesitas? Escríbenos e incluye tu modelo de dispositivo y versión de iOS/Android.",
    s_email_label:"Correo",
    s_email_value:"neonswipe-support@gmail.com",
    ...
    p_local_d:"Toda la revisión de fotos y vídeos ocurre localmente en tu dispositivo. No subimos ni almacenamos tu biblioteca en nuestros servidores, ni vemos tu contenido.",
    p_perm_t:"Permisos",
    p_perm_d:"El acceso a tu Fototeca se solicita únicamente para mostrar elementos y realizar las acciones que elijas (conservar, borrar, favorito). Puedes cambiar o revocar el permiso en Ajustes del sistema.",
    p_ads_t:"Publicidad y conectividad",
    p_ads_d:"La app necesita conexión a internet para cargar anuncios. Tu contenido nunca se envía con las solicitudes de anuncios. Los proveedores pueden recibir identificadores del dispositivo e información general (idioma, región) para ofrecer y medir anuncios según sus políticas.",
    p_iap_t:"Compras dentro de la app",
    p_iap_d:"Las suscripciones y compras las procesan Apple o Google. Pueden mantener metadatos y recibos de compra conforme a sus políticas de privacidad. No recibimos tus datos de pago completos.",
    p_data_t:"Datos que no recopilamos",
    p_data_d:"No recopilamos en nuestros servidores tus archivos de fotos/vídeos, álbumes, favoritos ni la estructura de tu biblioteca. Las acciones (conservar/borrar) permanecen en tu dispositivo.",
    p_diag_t:"Diagnósticos",
    p_diag_d:"Si aceptas compartir diagnósticos con Apple/Google o nos envías registros por soporte, podemos recibir informes anónimos de fallos y datos técnicos para corregir errores y mejorar el rendimiento.",
    p_security_t:"Seguridad",
    p_security_d:"Aplicamos salvaguardas estándar del sector a los datos limitados que mantenemos (p. ej., correos de soporte). Cuando es posible, los datos se cifran en tránsito y en reposo.",
    p_third_t:"Terceros",
    p_third_d:"Cuando interactúas con anuncios o realizas una compra, los socios correspondientes (redes publicitarias, Apple, Google) procesan tus datos según sus propias políticas.",
    p_region_t:"Transferencias internacionales",
    p_region_d:"Debido a que la publicidad y la facturación las prestan plataformas globales, tu información puede procesarse fuera de tu país. Esos proveedores aplican sus propias garantías para dichas transferencias.",
    p_ret_t:"Conservación",
    p_ret_d:"Los correos de soporte y la correspondencia relacionada se conservan solo el tiempo necesario para atender tu solicitud y cumplir obligaciones legales.",
    p_changes_t:"Cambios en esta política",
    p_changes_d:"Podemos actualizar este aviso. Los cambios relevantes se publicarán aquí con una nueva fecha de vigencia.",
    p_contact_t:"Contacto",
    p_contact_d:'¿Preguntas sobre privacidad? Escribe a <a href="mailto:neonswipe-support@gmail.com">neonswipe-support@gmail.com</a>.',
    p_effective_t:"Fecha de vigencia",
    p_effective_d:"Esta política entra en vigor el 1 de octubre de 2025."
  }
};
...
function setLocale(loc){
  ...
  // Replace all translatable nodes
  ...
  // Swap store badges (exists only on home)
  const iosImg = $("#badge-ios"), andImg = $("#badge-android");
  if (iosImg){
    if (loc==="es" && window.locales.es_has_badges) { iosImg.src="img/badge-appstore-es.png"; iosImg.alt="Descárgalo en el App Store"; }
    else { iosImg.src="img/badge-appstore-en.png"; iosImg.alt="Download on the App Store"; }
  }
  if (andImg){
    if (loc==="es" && window.locales.es_has_badges) { andImg.src="img/badge-google-es.png"; andImg.alt="Disponible en Google Play"; }
    else { andImg.src="img/badge-google-en.png"; andImg.alt="Get it on Google Play"; }
  }

  // Update pricing label only if present (prevents subpage errors)
  ...
  renderPrices();  // will no-op on subpages
}

function renderPrices(){
  const sym = $("#curSymbol"), amountEl = $("#priceAmount"), per = $("#pricePer");
  if (!sym || !amountEl || !per) return; // subpages: nothing to render
...
function countUp(el, to=9.82, secs=5.4){
  if(!el) return;
...
function initInfiniteRail(rail){
  if(!rail) return;
  rail.innerHTML += rail.innerHTML; // duplicate for seamless loop
...
(function init(){
  ...
  // Prefer store button order on home
  const iosBtn=$("#btn-ios"), andBtn=$("#btn-android");
  if(iosBtn && andBtn){
    const isIOS = /iPad|iPhone|iPod|Macintosh/.test(navigator.userAgent);
    iosBtn.style.order = isIOS ? 0 : 1;
    andBtn.style.order = isIOS ? 1 : 0;
  }
  ...
  // Space-saved counter on home
  const gb = $("#gbCount");
  if (gb){
    const io = new IntersectionObserver((ent)=>{ if(ent[0].isIntersecting){ countUp(gb, 9.82, 5.4); io.disconnect(); }},{threshold:0.4});
    io.observe(gb);
  }

  // Pricing controls (home only)
  billing.currency = detectCurrency();
  const curEUR=$("#curEUR"), curUSD=$("#curUSD"), billMonthlyEl=$("#billMonthly"), billAnnualEl=$("#billAnnual");
  if(curEUR && curUSD){ (billing.currency==="USD" ? curUSD : curEUR).classList.add("is-active"); }
  if(billMonthlyEl) billMonthlyEl.classList.add("is-active");
  renderPrices();
  billMonthlyEl?.addEventListener("click", ()=>{ billing.period="monthly"; billMonthlyEl.classList.add("is-active"); billAnnualEl?.classList.remove("is-active"); renderPrices(); });
  billAnnualEl?.addEventListener("click", ()=>{ billing.period="annual"; billAnnualEl.classList.add("is-active"); billMonthlyEl?.classList.remove("is-active"); renderPrices(); });
  ...
  // Auto-loop feature rail (home only)
